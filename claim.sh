...
# 执行第一个命令并提取账户地址
output=$(./../client/qclient-2.0.2.3-linux-amd64 token balance)
echo "$output"
...
# 执行第二个命令
output2=$(./../client/qclient-2.0.2.3-linux-amd64 cross-mint "$param1")
echo "$output2"
...
# 执行第三个命令
output3=$(./../client/qclient-2.0.2.3-linux-amd64 cross-mint "$param2")
echo "$output3"
